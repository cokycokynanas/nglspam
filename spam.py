#!/usr/bin/env python3
# spam.py — NGL Spam Sender (Multi-Target, Stable) by Maklu
# Versi dengan Smart Delay (Proteksi IP) dan Fitur Loop Akhir
# MODIFIKASI: Menambahkan tampilan Selamat Datang Tuan Boysz
...
# --- Variabel Nama Pengguna (Bisa diubah) ---
USER_NAME = "boysz" 
...
def print_header():
    ...
    # Divider/Garis Pemisah Keren
    print(f"{MAGENTA}{BOLD}{'=' * 60}{RESET}")
    print(f"{CYAN}{BOLD}>>> NGL.LINK AUTOMATED SENDER v2.5 <<<{RESET}")
    
    # Pesan Selamat Datang yang dipersonalisasi
    welcome_message = f"SELAMAT DATANG TUAN {USER_NAME.upper()}"
    # Menghitung padding agar pesan berada di tengah
    padding = (60 - len(welcome_message) - 2) // 2 
    
    # Baris Selamat Datang dengan efek tilde (~) dan warna
    print(f"{BLUE}{'~' * padding}{GREEN} {BOLD}{welcome_message} {BLUE}{'~' * (60 - len(welcome_message) - 2 - padding)}{RESET}")
...
def get_adaptive_delay(base_delay):
    """Menghasilkan delay acak (Smart Delay) untuk menghindari deteksi bot"""
    # Delay akan bervariasi antara base_delay hingga base_delay + 3.0 detik
...
def run_spam_session():
    """Mengumpulkan input dan menjalankan loop spam"""
    
    print_header()

    # --- INPUT PENGGUNA ---
    global usernames, pesan, jumlah_per_target, delay
    ...
    delay = parse_float(f"{CYAN}⏳ Base Delay antar kiriman dalam detik (default 5.0): {RESET}", default=5.0)
    ...
    print(f"   Base Delay: {delay} detik (Smart Delay Aktif)\n")
    ...
    try:
        ...
        while True:
            ...
            current_delay = get_adaptive_delay(delay) # Ambil delay acak
            ...
            try:
                ...
                # --- PENANGANAN ERROR 429 (RATE LIMIT) ---
                if status == 429:
                    print(f"\r{RED}{BOLD}!!! RATE LIMIT (429) TERDETEKSI !!!{RESET}")
                    print(f"{YELLOW}   -> Server memblokir IP Anda sementara. Menunggu 60 detik sebelum lanjut...{RESET}")
                    time.sleep(60) 
                    continue # Langsung lanjut ke target berikutnya tanpa menambah 'sent'
                
                # Jika status bukan 429, hitung pesan sebagai terkirim
                sent += 1
                ...
                if qid:
                    ...
                    print(f"{DIM}   -> Response: {response_text}{RESET}")
                
            except Exception as e:
                print(f"\r{RED}[{now}] ❌ Error koneksi ke {current_username}: {e}{RESET}")
                
            # Tampilkan progress bar lagi setelah hasil
            if total_sent_limit > 0:
                print_progress(sent, total_sent_limit)
            
            # Jeda antar target (menggunakan current_delay yang acak)
            sys.stdout.write(f"\n{DIM}   ⏳ Jeda {current_delay:.1f}s sebelum target berikutnya (Smart Delay)...{RESET}\r")
            sys.stdout.flush()
        ...
        return # Keluar dari fungsi sesi spam

    return # Keluar dari fungsi sesi spam setelah selesai
...
if __name__ == "__main__":
    ...
    while True:
        ...
        # --- FITUR LOOP AKHIR ---
        print("\n" + MAGENTA + "--- SESI SELESAI ---" + RESET)
        choice = input(f"{CYAN}Apakah Anda ingin mengulangi sesi spam? ({BOLD}Y{RESET}{CYAN}/{BOLD}N{RESET}{CYAN}): {RESET}").strip().lower()

        if choice != 'y':
            print(GREEN + "Terima kasih, program diakhiri." + RESET)
            break
        else:
            # Ulangi sesi (Kembali ke awal loop while True)
            print(f"\n{BLUE}{BOLD}Memulai sesi baru...{RESET}")
